...
export interface BoardProps {
  ...
  /**
   * Rotates the wall's rendered starting corner (Todo B: purely cosmetic --
   * randomised once per hand by App so the wall doesn't always break at the
   * same corner). Undefined/0 keeps the original top-left start.
   */
  readonly wallStartOffset?: number | undefined;
}
  ...
  return (
    ...
    >
      ...
      <div className={styles.slotCentre}>
        <DiscardArea state={state} wallStartOffset={wallStartOffset} />
      </div>
...
function DiscardArea({ state, wallStartOffset }: { state: GameState; wallStartOffset?: number | undefined }) {
  ...
  // Discard visibility (MJrules.md "Discard Visibility"): default is a face-up
  // pool. In hard mode (config.discardsVisible === false) only the tile just
  // played stays visible, and only while the claim window is open on it --
  // once the window closes it joins the face-down pool like the rest.
  const revealTile = (tileId: string) =>
    state.config.discardsVisible ||
    (tileId === lastDiscardId && state.phase === 'CLAIM_WINDOW');
  ...
  return (
    <div className={styles.centre}>
      ...
      <WallFrame liveCount={wall.live.length} deadCount={wall.dead.length} startOffset={wallStartOffset}>
          ...
          {w > 0 && discardPool.map((tile, i) => (
            <div key={tile.id} className={styles.discardTile} style={discardStyle(tile.id, i, cols, rows, w, h)}>
              <Tile
                tile={tile}
                size={DISCARD_TILE}
                faceDown={!revealTile(tile.id)}
                highlight={tile.id === lastDiscardId ? 'red' : undefined}
              />
            </div>
          ))}
        </div>
